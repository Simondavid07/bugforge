...
export function applyProjectAccent(
...
export default function ProjectPersonalization() {
  ...
  const preferences = trpc.personalization.get.useQuery(undefined, {
    retry: false,
  });
  ...
  const upload = trpc.personalization.uploadImage.useMutation({
    onSuccess: async () => {
      await Promise.all([
        utils.workspace.mine.invalidate(),
        utils.personalization.get.invalidate(),
      ]);
    },
  });
  const [deleteWorkspaceId, setDeleteWorkspaceId] = useState<number | null>(
    null
  );
  ...
  const deleteWorkspace = trpc.workspace.delete.useMutation({
    onSuccess: async () => {
      ...
      localStorage.removeItem("bugforge-active-project");
      window.location.assign("/");
    },
  });
  const [motion, setMotion] = useState<MotionLevel>(
    () => (localStorage.getItem("bugforge-motion") as MotionLevel) || "soft"
  );
  ...
  useEffect(() => {
    ...
    const items = Array.from(
      menu.querySelectorAll<HTMLElement>("[data-slot='sidebar-menu-item']")
    );
    const byPath = new Map(
      items.map(item => {
        const text = item.textContent?.trim() ?? "";
        const path = Object.entries(sidebarLabels).find(([, label]) =>
          text.includes(label)
        )?.[0];
  ...
  const persist = (
    nextSidebar: string[] = sidebarOrder,
    nextProjects: number[] = projectOrder
  ) =>
    updatePreferences.mutate({
      sidebarOrder: nextSidebar as [
        "/",
        "/issues",
        "/boards",
        "/analytics",
  ...
  const moveSidebar = (path: string, direction: -1 | 1) => {
    ...
    if (next < 0 || next >= sidebarOrder.length)
      return announce(
        `${label} is already at the ${direction < 0 ? "top" : "bottom"}.`
      );
  ...
  const moveProject = (id: number, direction: -1 | 1) => {
    ...
    if (!project) return;
    if (next < 0 || next >= orderedProjects.length)
      return announce(
        `${project.name} is already at the ${direction < 0 ? "top" : "bottom"}.`
      );
    const order = orderedProjects.map(item => item.id);
    [order[current], order[next]] = [order[next], order[current]];
    persist(sidebarOrder, order);
    announce(
      `Moved ${project.name} to position ${next + 1} of ${order.length}.`
    );
  };
  ...
  const dropProject = (event: DragEvent<HTMLButtonElement>, target: number) => {
    ...
    persist(
      sidebarOrder,
      reorder(current.map(String), dragged, String(target)).map(Number)
    );
    setDragged(null);
  };
  const onUpload = async (
    event: ChangeEvent<HTMLInputElement>,
    target: "avatar" | "project_logo"
  ) => {
    ...
    upload.mutate({
      target,
      ...(target === "project_logo" && projectId ? { projectId } : {}),
      fileName: file.name,
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      )?.closest<HTMLButtonElement>("button[draggable]");
      if (!button) return;
      event.preventDefault();
      ...
      <div className="mt-3 max-h-[75vh] w-80 overflow-y-auto rounded-2xl border border-[#D8CDC0] bg-[#FFFCF7] p-4 shadow-[0_18px_45px_rgba(73,52,38,.18)] dark:border-[#4A433D] dark:bg-[#211F1D]">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-[#665D56] dark:text-[#C9BEB4]">
              Personalize appearance, order, and pace.
            </p>
          </div>
        ...
        <div className="mt-5 space-y-2">
          ...
          <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-[#D8CDC0] p-3 text-xs dark:border-[#4A433D]">
            <span className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl bg-[#EFE5D7] dark:bg-[#2A2623]">
              {preferences.data?.avatarUrl ? (
                <img
                  src={preferences.data.avatarUrl}
                  alt="Your avatar"
            ...
            <span className="flex-1">
              <span className="block font-semibold">Your avatar</span>
              <span className="mt-0.5 block text-[#665D56] dark:text-[#C9BEB4]">
                PNG, JPG, or WebP · 2 MB
              </span>
            </span>
            ...
            <input
              ...
              onChange={event => onUpload(event, "avatar")}
            />
          </label>
          <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-[#D8CDC0] p-3 text-xs dark:border-[#4A433D]">
            <span className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl bg-[#EFE5D7] dark:bg-[#2A2623]">
            ...
            <span className="flex-1">
              <span className="block font-semibold">Project logo</span>
              <span className="mt-0.5 block text-[#665D56] dark:text-[#C9BEB4]">
                Admins can update this mark
              </span>
            </span>
            <ImageUp className="h-4 w-4" />
            <input
              ...
              type="file"
              accept="image/png,image/jpeg,image/webp"
              onChange={event => onUpload(event, "project_logo")}
            />
          </label>
          {upload.error && (
            <p className="text-xs text-[#A55343]">{upload.error.message}</p>
          )}
        </div>
        <div className="mt-5 border-t border-[#D8CDC0] pt-4 dark:border-[#4A433D]">
          ...
          {!projectId ? (
            <p className="mt-1 text-[11px] text-[#665D56] dark:text-[#C9BEB4]">
              Create or choose a project first; its accent controls will then
              unlock here.
            </p>
          ) : (
            <p className="mt-1 text-[11px] text-[#665D56] dark:text-[#C9BEB4]">
        ...
        <div className="mt-5 border-t border-[#D8CDC0] pt-4 dark:border-[#4A433D]">
          ...
          <p className="mt-1 text-[11px] text-[#665D56] dark:text-[#C9BEB4]">
            Drag to reorder, use Alt + ↑ / ↓, or the move buttons.
          </p>
          <div className="mt-2 space-y-1">
            {sidebarOrder.map((path, index) => (
              <div
                key={path}
                className="flex items-center gap-1 rounded-lg hover:bg-[#EFE5D7] dark:hover:bg-[#2A2623]"
              >
                <button
                  ...
                  onDrop={event => dropSidebar(event, path)}
                  className="flex min-w-0 flex-1 items-center gap-2 rounded-lg px-2 py-1.5 text-left text-xs focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#A55343]"
                  aria-keyshortcuts="Alt+ArrowUp Alt+ArrowDown"
                >
                  <GripVertical className="h-3.5 w-3.5 shrink-0 text-[#8A978F]" />
                  {sidebarLabels[path]}
                </button>
                <button
                  type="button"
                  aria-label={`Move ${sidebarLabels[path]} up`}
                  title={`Move ${sidebarLabels[path]} up`}
                  disabled={index === 0}
                  onClick={() => moveSidebar(path, -1)}
                  className="rounded-md p-1 text-[#665D56] hover:bg-white disabled:cursor-not-allowed disabled:opacity-35 dark:text-[#C9BEB4] dark:hover:bg-[#3A3530]"
                >
                  <ChevronUp className="h-3.5 w-3.5" />
                </button>
                <button
                  type="button"
                  aria-label={`Move ${sidebarLabels[path]} down`}
                  title={`Move ${sidebarLabels[path]} down`}
          ...
          <p className="mt-3 text-[11px] text-[#665D56] dark:text-[#C9BEB4]">
            Project order also changes Cmd/Ctrl+K results.
          </p>
          <div className="mt-2 space-y-1">
            {orderedProjects.map((project, index) => (
              ...
              >
                <button
                  ...
                  draggable
                  onDragStart={() => setDragged(String(project.id))}
                  onDragOver={event => event.preventDefault()}
        ...
        <div className="mt-5 border-t border-[#D8CDC0] pt-4 dark:border-[#4A433D]">
          ...
          <p className="mt-1 text-[11px] text-[#665D56] dark:text-[#C9BEB4]">
            Workspace admins can permanently remove a workspace and all of its
            projects, issues, and members.
          </p>
          <div className="mt-3 space-y-2">
            {(workspace?.workspaces ?? []).map(item => {
              const isDeleting = deleteWorkspaceId === item.workspaceId;
              const canDelete = item.role === "admin";
              return (
                <div
                  key={item.workspaceId}
                  className="rounded-xl border border-[#D8CDC0] p-3 dark:border-[#4A433D]"
                >
                  <div className="flex items-center gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-[10px] uppercase tracking-wide text-[#8A978F]">
                        {item.role}
                      </p>
                    </div>
                    {canDelete && (
                      <Button
                        ...
                        onClick={() => {
                          setDeleteWorkspaceId(
                            isDeleting ? null : item.workspaceId
                          );
                          setDeleteConfirmation("");
                        }}
                        className="h-7 rounded-lg px-2 text-[10px] text-[#A55343] hover:bg-[#FFF1EF]"
                      >
                  ...
                  {isDeleting && (
                    <div className="mt-3 space-y-2">
                      <label
                        className="block text-[10px] font-medium text-[#665D56] dark:text-[#C9BEB4]"
                        htmlFor={`delete-workspace-${item.workspaceId}`}
                      >
                        Type <span className="font-semibold">{item.name}</span>{" "}
                        to confirm
                      </label>
                      <input
                        id={`delete-workspace-${item.workspaceId}`}
                        value={deleteConfirmation}
                        onChange={event =>
                          setDeleteConfirmation(event.target.value)
                        }
                        className="h-8 w-full rounded-lg border border-[#D8CDC0] bg-transparent px-2 text-xs outline-none focus:border-[#A55343] dark:border-[#4A433D]"
                        autoComplete="off"
                      />
                      <Button
                        type="button"
                        ...
                        className="h-8 w-full rounded-lg bg-[#A55343] text-[11px] text-white hover:bg-[#8F463A] disabled:opacity-50"
                      >
                        {deleteWorkspace.isPending
                          ? "Deleting workspace…"
                          : "Delete permanently"}
        ...
        <div className="mt-5 border-t border-[#D8CDC0] pt-4 dark:border-[#4A433D]">
          ...
          <p className="mt-1 text-[11px] text-[#665D56] dark:text-[#C9BEB4]">
            Still removes ambient movement. Soft is balanced. Expressive adds
            playful feedback.
          </p>
          <div className="mt-2 grid grid-cols-3 gap-1 rounded-xl bg-[#EFE5D7] p-1 dark:bg-[#2A2623]">
            {(["still", "soft", "expressive"] as MotionLevel[]).map(level => (
              <Button
                key={level}
                type="button"
                variant="ghost"
                onClick={() => setMotion(level)}
                className={`h-8 rounded-lg text-[11px] capitalize ${motion === level ? "bg-white text-[#1E1A18] shadow-sm dark:bg-[#3A3530] dark:text-[#F4EEE6]" : "text-[#665D56] dark:text-[#C9BEB4]"}`}
              >
                {level}
              </Button>
            ))}
