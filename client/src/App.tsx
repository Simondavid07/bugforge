...
const ProjectPersonalization = lazy(() => import("@/components/ProjectPersonalization"));
...
function WorkspaceRoute({ component: Component }: { component: ElementType }) { return <DashboardLayout><CorrespondenceFrame><Suspense fallback={<div className="grid min-h-[45vh] place-items-center text-sm text-[#665D56] dark:text-[#C9BEB4]">Opening your workspace…</div>}><Component /></Suspense></CorrespondenceFrame></DashboardLayout>; }
...
export default function App() { return <ErrorBoundary><ThemeProvider defaultTheme="light" switchable><TooltipProvider><Toaster richColors /><Router /><Suspense fallback={null}><CommandPalette /><ProjectPersonalization /></Suspense><div id="personalization-announcer" className="sr-only" aria-live="polite" aria-atomic="true" /></TooltipProvider></ThemeProvider></ErrorBoundary>; }
