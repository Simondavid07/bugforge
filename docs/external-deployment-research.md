...
| External AI Gateway | Do not activate an external model provider, provider credential, model funding, or billable request without a separate owner decision. Keep the current managed AI recommendation path unchanged. | User-directed deployment boundary |
...
The optional Vercel AI Gateway adapter was not activated. By user direction, the external deployment remains free of AI provider keys, funding configuration, and billable model calls, while the existing managed AI recommendation behavior remains unchanged.

> The Vercel and Supabase guidance supports the current source layout, private Storage model, and GitHub Supabase Auth route. It does not authorize a model request, a payment method, or removal of the managed rollback runtime; those remain owner-controlled decisions.
